...
class EspritTUIApp(App):  # type: ignore[misc]
    ...
    def _render_tool_content_simple(self, tool_data: dict[str, Any]) -> str:
        ...
        # Status dot colors: green=completed, white=running, red=failed
        status_dots = {
            "running": "[white]●[/white]",
        ...
        dot = status_dots.get(status, "[dim]●[/dim]")
        ...
        display_names = {
            ...
            "agents_graph_action": "Agents",
            "file_edit_action": "Edit",
            "proxy_action": "Proxy",
            "notes_action": "Notes",
            "thinking_action": "Thinking",
            "web_search_action": "Search",
            "finish_action": "Finish",
            "reporting_action": "Report",
            "scan_start_info": "Scan",
            "subagent_start_info": "Subagent",
            "llm_error_details": "LLM Error",
        }
        display_name = display_names.get(tool_name, tool_name)

        # Primary argument to show for each tool type
        primary_arg_keys = {
            "terminal_execute": "command",
            "browser_action": "action",
            "python_action": "code",
            "file_edit_action": "file_path",
            "web_search_action": "query",
            "notes_action": "action",
            "thinking_action": "thought",
            "reporting_action": "action",
        }

        # Get the primary argument value
        primary_key = primary_arg_keys.get(tool_name)
        primary_value = ""
        if primary_key and primary_key in args:
            primary_value = str(args[primary_key])
            # Truncate long values
            if len(primary_value) > 50:
                primary_value = primary_value[:47] + "..."
            # Remove newlines for compact display
            primary_value = primary_value.replace("\n", " ").strip()
            primary_value = escape_markup(primary_value)

        # Build compact single-line output
        if primary_value:
            return f"{dot} {display_name}({primary_value})"
        else:
            return f"{dot} {display_name}"
