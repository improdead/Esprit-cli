...
- esprit login [--provider github|google]
- esprit logout
- esprit whoami
- esprit status
- esprit scan --target <target> [options]
"""
...
from esprit.interface.cli import run_cli
...
from esprit.runtime.docker_runtime import ESPRIT_IMAGE
...
def validate_environment() -> None:  # noqa: PLR0912, PLR0915
    ...
    if missing_required_vars:
        ...
        panel = Panel(
            error_text,
            title="[bold red]🛡️  ESPRIT CONFIGURATION ERROR",
            title_align="center",
            border_style="red",
            padding=(1, 2),
        )
...
async def warm_up_llm() -> None:
    ...
    try:
        model_name = os.getenv("ESPRIT_LLM", "openai/gpt-5")
...
def parse_arguments() -> argparse.Namespace:
    ...
    login_parser = subparsers.add_parser(
        "login",
        help="Login to Esprit (opens browser for OAuth)",
    )
    login_parser.add_argument(
        ...
        default="github",
        help="OAuth provider to use (default: github)",
    )
    ...
    # Scan command (existing functionality)
    scan_parser = subparsers.add_parser(
        "scan",
        help="Run a penetration test",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  ...
  # Multiple targets
  esprit scan --target https://github.com/user/repo --target https://example.com

  # Custom instructions
  esprit scan --target example.com --instruction "Focus on authentication vulnerabilities"
        """,
    )
    ...
    # For backward compatibility, also allow the old syntax without subcommand
    parser.add_argument(
        "-t",
        "--target",
        type=str,
    ...
    # Handle backward compatibility: if --target is used without subcommand
    if args.command is None and args.target:
        args.command = "scan"

    # If no command and no target, show help
    if args.command is None:
        parser.print_help()
        sys.exit(0)

    # Process scan command arguments
    if args.command == "scan":
        if not args.target:
            scan_parser.error("the following arguments are required: -t/--target")
                ...
                try:
                    with instruction_path.open(encoding="utf-8") as f:
                        ...
                        if not args.instruction:
                            scan_parser.error(f"Instruction file '{instruction_path}' is empty")
                except Exception as e:  # noqa: BLE001
                    scan_parser.error(f"Failed to read instruction file '{instruction_path}': {e}")
        ...
        for target in args.target:
            try:
                ...
                scan_parser.error(f"Invalid target '{target}'")
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    ...
    panel = Panel(
        panel_content,
        title="[bold green]🛡️  ESPRIT CYBERSECURITY AGENT",
        title_align="center",
...
def pull_docker_image() -> None:
    ...
    if image_exists(client, ESPRIT_IMAGE):
        return

    console.print()
    console.print(f"[bold cyan]🐳 Pulling Docker image:[/] {ESPRIT_IMAGE}")
    ...
    with console.status("[bold cyan]Downloading image layers...", spinner="dots") as status:
        try:
            ...
            for line in client.api.pull(ESPRIT_IMAGE, stream=True, decode=True):
            ...
            error_text.append("\n\n", style="white")
            error_text.append(f"Could not download: {ESPRIT_IMAGE}\n", style="white")
...
def run_scan(args: argparse.Namespace) -> None:
    """Run the scan command (original main functionality)."""
...
def main() -> None:
    ...
    if args.command == "login":
        from esprit.auth.commands import cmd_login

        sys.exit(cmd_login(args.provider))

    elif args.command == "logout":
        from esprit.auth.commands import cmd_logout

        sys.exit(cmd_logout())

    elif args.command == "whoami":
        from esprit.auth.commands import cmd_whoami

        sys.exit(cmd_whoami())

    elif args.command == "status":
        from esprit.auth.commands import cmd_status
