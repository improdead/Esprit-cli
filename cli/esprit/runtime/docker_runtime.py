...
class DockerRuntime(AbstractRuntime):
    ...
    def _initialize_container(
        ...
        # Entrypoint already polls Caido for up to 30s, no additional sleep needed
        ...
        self._wait_for_tool_server(tool_server_port)

    def _wait_for_tool_server(
        self, port: int, max_attempts: int = 50, interval: float = 0.1
    ) -> None:
        """Poll tool_server /health endpoint until ready."""
        for attempt in range(max_attempts):
            try:
                with httpx.Client() as client:
                    resp = client.get(f"http://localhost:{port}/health", timeout=0.5)
                    if resp.status_code == 200:
                        logger.info("Tool server ready after %d attempts", attempt + 1)
                        return
        ...
        logger.warning(
            "Tool server health check timed out after %.1fs, proceeding anyway",
